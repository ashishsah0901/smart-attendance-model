...
# print(type(image0))
    ...
    for i in range(14):
        ...
        euclidean_distance = F.pairwise_distance(output1, output2)
        if(label2.item() == 0.0):
            if(euclidean_distance.item() <= 1.2):
                correct += 1;
            else:
                incorrect += 1;
        elif(label2.item() == 1.0):
            if(euclidean_distance.item() > 1.2):
                correct += 1;
            else:
                incorrect += 1;
        ...
        imshow(torchvision.utils.make_grid(concatenated),'Dissimilarity: {:.2f}, Lable: {:.2f}'.format(euclidean_distance.item(), label2.item()))
